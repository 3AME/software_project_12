...
import agriculture.dao.Log;
import agriculture.dao.LogDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String colOperation=request.getParameter("col_operation");//查询功能使用
        String colUserId=request.getParameter("col_user_id");
        String colSub=request.getParameter("col_sub");
        String ip=request.getParameter("ip");//查询功能使用
        ...
        query.setColOperation(colOperation);
        query.setColUserId(colUserId);
        query.setColSub(colSub);
        query.setIp(ip);
        ...
        if(existResultset.equals("1")){
            ...
            jsonObj=logDao.getRecord(query);
    ...
    public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String colTime=request.getParameter("col_time");
        String colContent=request.getParameter("col_content");
        String colOperation=request.getParameter("col_operation");
        String colUserId=request.getParameter("col_user_id");
        String colModule=request.getParameter("col_module");
        String colSub=request.getParameter("col_sub");
        ...
        if(dbName!=null){
            ...
            log.setColTime(colTime);
            log.setColContent(colContent);
            log.setColOperation(colOperation);
            log.setColUserId(colUserId);
            log.setColModule(colModule);
            log.setColSub(colSub);
            log.setIp(ip);
            log.setCreateTime(createTime);

            jsonObj=logDao.addRecord(log);
            ylxLog.log("用户 "+creator+" 于 "+createTime+" 添加了 ["+module+"]["+sub+"] 记录，选择的时间为["+colTime+"]","添加记录",module,sub);
        }
        boolean isAjax=true;
        if(request.getHeader("x-requested-with")==null){isAjax=false;}	//判断是异步请求还是同步请求
    ...
    public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id!=null && dbName!=null){
            ...
            jsonObj=logDao.modifyRecord(log);
...
//            response.sendRedirect(url);
    ...
    public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
            LogDao logDao=new LogDao();
            jsonObj=logDao.deleteRecord(action,dbName,ids,creator,createTime);
    ...
    public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
            LogDao logDao=new LogDao();
            jsonObj=logDao.setRecordTop(action,dbName,type,userId,id);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        boolean isAjax=true;
    ...
    public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String creator=(String)session.getAttribute("user_name");
        String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        //String exportFilePathName="C:\\upload\\agriculture\\export\\log_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
        String exportFilePathName="C:\\download\\XM18\\";
        String filename="log_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
    ...
    public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
        if(jsonObj!=null){
            ...
            //需要观察json里的格式
            String[] cols={"role_id⊙text⊙id","time_interval⊙text⊙时间","count⊙text⊙内容","color⊙text⊙操作","color_name⊙text⊙操作人",
                    "color_name⊙text⊙项目","color_name⊙text⊙子模块","color_name⊙text⊙IP地址"};	//name⊙type⊙nick
