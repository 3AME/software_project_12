...
public class AlarmDao {
	 ...
	 * 功能：返回结果集
	 */
	public JSONObject getRecord(Alarm query) throws SQLException, IOException, JSONException {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(rs.getString("value_string"));
				list.add(rs.getString("value_type"));

				list.add(rs.getString("handle_status"));
				list.add(rs.getString("send_status"));
				list.add(rs.getString("handle_time"));
	...
	public JSONObject modifyRecord(Alarm alarm) throws JSONException{
		//String action,String dbName,String id,String title(user_name),String password,String creator,String createTime
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		try {
			DB query_db = new DB(alarm.getDbName());
			//构造sql语句，根据传递过来的查询条件参数
			String tableName="agriculture_alarm";
			String sql="update "
					+tableName
					+" set value_string='"
					+alarm.getValueString()
					+"',value_float='"
					+Float.parseFloat(alarm.getValueString())
					+"',value_type='"
					+alarm.getValueType()
					+"',handle_status='"
					+alarm.getHandleStatus()
					+"',send_status='"
					+alarm.getSendStatus()
					+"' where id="
					+alarm.getId();

			System.out.println("sql in modifyRecord in alarmDao is "+sql);
		...
		JSONObject jsonObj=new JSONObject();
		jsonObj.put("aaData",jsonList);
		jsonObj.put("action",alarm.getAction());
		jsonObj.put("result_msg",resultMsg);//如果发生错误就设置成"error"等
		jsonObj.put("result_code",resultCode);//返回0表示正常，不等于0就表示有错误产生，错误代码
		return jsonObj;
	}
	...
	public JSONObject addRecord(Alarm alarm) throws JSONException, SQLException{
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		DB query_db = new DB(alarm.getDbName());
		//构造sql语句，根据传递过来的查询条件参数
		String tableName="agriculture_alarm";
		String sql="insert into "+tableName+"(value_string,value_float,value_type,handle_status,send_status,handle_time,creator,create_time) values('"
				+alarm.getValueString()+"','"
				+Float.parseFloat(alarm.getValueString())+"','"
				+alarm.getValueType()+"','"
				+Integer.parseInt(alarm.getHandleStatus())+"','"
				+Integer.parseInt(alarm.getSendStatus())+"','"
				+alarm.getHandleTime()+"','"
				+alarm.getCreator()+"','"
				+alarm.getCreateTime()+"')";

		System.out.println("sql in addRecord in AlarmDao is "+sql);
		query_db.executeUpdate(sql);
		query_db.close();
		//下面开始构建返回的json
		JSONObject jsonObj=new JSONObject();
	...
	public JSONObject setRecordTop(String action,String dbName,String type,String userId,String id) throws JSONException, SQLException{
		...
		//构造sql语句，根据传递过来的查询条件参数
		//String sql="select max(priority) as priority from agriculture_alarm where user_id='"+userId+"'";
		String sql="select max(priority) as priority from agriculture_alarm";
		int priority=0;
		ResultSet rs=query_db.executeQuery(sql);
		if(rs.next()){
		...
		query_db.close();
		update_db.executeUpdate("update agriculture_alarm set priority="+(priority+1)+" where id="+id);
		update_db.close();
		//下面开始构建返回的json
		JSONObject jsonObj=new JSONObject();
		jsonObj.put("aaData",jsonList);
	 ...
	 */
	private String createGetRecordSql(Alarm query){
		String sql="";
		String where="";
...
/*
		...
		*/
        ...
        //构造排序条件orderby开始
        String orderBy="";
		System.out.println("orderBy in alarmDao is "+query.getOrderBy());
