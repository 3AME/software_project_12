...
import agriculture.dao.Users;
import agriculture.dao.UsersDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            users.setDbName(dbName);
            users.setParentId(agricultureId);
            users.setTitle(title);
            users.setPassword(password);
            users.setLimitTime(limitTime);
            users.setUserId(userId);
            users.setCreator(creator);
            users.setCreateTime(createTime);
            users.setRoleId(roleId);
            users.setEmail(email);
            jsonObj=usersDao.addRecord(users);
    ...
    public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id!=null && dbName!=null){
            ...
            users.setCreator(creator);
            users.setCreateTime(createTime);
            jsonObj=usersDao.modifyRecord(users);
...
//            response.sendRedirect(url);
    ...
    public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
            UsersDao usersDao=new UsersDao();
            jsonObj=usersDao.deleteRecord(action,dbName,ids,creator,createTime);
    ...
    public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
            UsersDao usersDao=new UsersDao();
            jsonObj=usersDao.setRecordTop(action,dbName,type,userId,id);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        boolean isAjax=true;
    ...
    public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String creator=(String)session.getAttribute("user_name");
        String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        //String exportFilePathName="C:\\upload\\agriculture\\export\\users_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
        String exportFilePathName="C:\\download\\XM18\\";
        String filename="users_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
    ...
    public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
        if(jsonObj!=null){
            ...
            //需要观察json里的格式
            String[] cols={"role_id⊙text⊙id","time_interval⊙text⊙uese_name","count⊙text⊙password","color⊙text⊙email","color_name⊙text⊙register_time",
                    "color_name⊙text⊙end_time","color_name⊙text⊙end_tag","color_name⊙text⊙user_id","color_name⊙text⊙creator","color_name⊙text⊙create_time",
                    "color_name⊙text⊙role_id"};	//name⊙type⊙nick
