package agriculture.weather;

import agriculture.dao.Weather;
import agriculture.dao.WeatherDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String detailString=request.getParameter("detail_string");

        String location=request.getParameter("location");//查询功能使用

        System.out.println("the location in servletaction is "+location);


        System.out.println("orderby in getRecord in weather servletAction: "+orderBy);
        ...
        query.setDetailString(detailString);
        ...
        if(existResultset.equals("1")){
            ...
            jsonObj=weatherDao.getRecord(query);

            session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
        }
        System.out.println("the json in getRecord in weather servletAction is "+jsonObj);
    ...
    public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        System.out.println("addRecord in weather servletAction entered!!!!");
        HttpSession session = request.getSession();
        String action = request.getParameter("action");
        String dbName=(String)session.getAttribute("unit_db_name");
        ...
        String detailString=request.getParameter("detail_string");
        String tempString=request.getParameter("temp_string");
        String windDirection=request.getParameter("wind_direction");
        String windForce=request.getParameter("wind_force");
        String humidityString=request.getParameter("humidity_string");
        String pressureString=request.getParameter("pressure_string");
        String precipitationString=request.getParameter("precipitation_string");
        ...
        if(dbName!=null){
            ...
            weather.setDbName(dbName);

            weather.setUserId(userId);
            weather.setCreator(creator);
            weather.setCreateTime(createTime);

            weather.setLocation(location);
            weather.setDetailString(detailString);
            weather.setTempString(tempString);
            weather.setWindDirection(windDirection);
            weather.setWindForce(windForce);
            weather.setHumidityString(humidityString);
            weather.setPressureString(pressureString);
            weather.setPrecipitationString(precipitationString);
            weather.setUpdateTime(updateTime);

            jsonObj=weatherDao.addRecord(weather);
    ...
    public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id!=null && dbName!=null){
            ...
            weather.setCreator(creator);
            weather.setCreateTime(createTime);
            jsonObj=weatherDao.modifyRecord(weather);
...
//            response.sendRedirect(url);
    ...
    public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
            WeatherDao weatherDao=new WeatherDao();
            jsonObj=weatherDao.deleteRecord(action,dbName,ids,creator,createTime);
    ...
    public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
            WeatherDao weatherDao=new WeatherDao();
            jsonObj=weatherDao.setRecordTop(action,dbName,type,userId,id);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        boolean isAjax=true;
    ...
    public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String creator=(String)session.getAttribute("user_name");
        String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        //String exportFilePathName="C:\\upload\\agriculture\\export\\weather_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
        String exportFilePathName="C:\\download\\XM18\\";
        String filename="weather_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
    ...
    public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
        if(jsonObj!=null){
            ...
            //需要观察json里的格式
            String[] cols={"role_id⊙text⊙id","time_interval⊙text⊙location","count⊙text⊙detail_string","color⊙text⊙temp_string","color_name⊙text⊙wind_direction",
                    "color_name⊙text⊙wind_force","color_name⊙text⊙humidity_string","color_name⊙text⊙pressure_string","color_name⊙text⊙precipitation_string","color_name⊙text⊙update_time",
