package agriculture.alarm;

import agriculture.dao.Alarm;
import agriculture.dao.AlarmDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String valueType=request.getParameter("value_type");//查询功能使用
        ...
        System.out.println("orderby in getRecord in alarm servletAction: "+orderBy);
        ...
        if(existResultset.equals("1")){
            ...
            jsonObj=alarmDao.getRecord(query);
            session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
        }
        System.out.println("the json in getRecord in alarm servletAction is "+jsonObj);
    ...
    public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String valueString=request.getParameter("value_string");
        String valueType=request.getParameter("value_type");
        String handleStatus=request.getParameter("handle_status");
        String sendStatus=request.getParameter("send_status");
        String handleTime=request.getParameter("handle_time");
        ...
        if(dbName!=null){
            ...
            alarm.setDbName(dbName);

            alarm.setUserId(userId);
            alarm.setCreator(creator);
            alarm.setCreateTime(createTime);

            alarm.setValueString(valueString);
            alarm.setValueType(valueType);
            alarm.setHandleStatus(handleStatus);
            alarm.setSendStatus(sendStatus);
            alarm.setHandleTime(handleTime);
            alarm.setValueType(valueType);
            jsonObj=alarmDao.addRecord(alarm);
    ...
    public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String valueString=request.getParameter("value_string");
        String valueType=request.getParameter("value_string");
        String handleStatus=request.getParameter("handle_status");
        String sendStatus=request.getParameter("send_status");
        ...
        if(id!=null && dbName!=null){
            ...
            alarm.setCreator(creator);
            alarm.setCreateTime(createTime);
            jsonObj=alarmDao.modifyRecord(alarm);
...
//            response.sendRedirect(url);
    ...
    public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
            AlarmDao alarmDao=new AlarmDao();
            jsonObj=alarmDao.deleteRecord(action,dbName,ids,creator,createTime);
    ...
    public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
            AlarmDao alarmDao=new AlarmDao();
            jsonObj=alarmDao.setRecordTop(action,dbName,type,userId,id);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        boolean isAjax=true;
    ...
    public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String creator=(String)session.getAttribute("user_name");
        String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        //String exportFilePathName="C:\\upload\\agriculture\\export\\alarm_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
        String exportFilePathName="C:\\download\\XM18\\";
        String filename="alarm_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
    ...
    public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
        if(jsonObj!=null){
            ...
            //需要观察json里的格式
            String[] cols={"role_id⊙text⊙id","time_interval⊙text⊙value_string","count⊙text⊙value_type","color⊙text⊙handle_status","color_name⊙text⊙send_status",
                    "color_name⊙text⊙handle_time","color_name⊙text⊙creator","color_name⊙text⊙create_time"};	//name⊙type⊙nick
