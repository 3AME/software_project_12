...
import agriculture.dao.Statistic;
import agriculture.dao.StatisticDao;
...
public class ServletAction extends HttpServlet {
	...
	public void statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String tableName=request.getParameter("table_name");//要统计的表名
        String moduleName=request.getParameter("module_name");//跳转到的页面名
		...
		if(dbName!=null){
            ...
            statistic.setTableName(tableName);
			/*---------------------------------把状态保存进session---------------------------*/
		...
		//String fileName=getStatisticGraph(session,dbName,jsonObj,"用户数据统计图","测试Column",statisticType,1,1280,768);/////
		...
		if(isAjax){
			...
			resultUrl=resultPath+"/"+moduleName+"_statistic_result.jsp";///
	...
	public String getStatisticGraph(HttpSession session,String dbName,JSONObject json,String title,String column,String statisticType,int statisticImage,int chartWidth,int chartHeight) throws SQLException, IOException, JSONException{
		...
		String tmpDir = "/chart";//tmpDir是虚拟出的路径
		...
		showDebug(arr.toString());////打印json数据
        ...
        showDebug("已经将json数据放入dataset中");//dataset是图要用到的参数
        ...
        showDebug(System.getProperty("java.io.tmpdir"));
		String chartFilename = ServletUtilities.saveChartAsJPEG(chart, chartWidth, chartHeight, null, session);
        showDebug(chartFilename);
		chartFilename = tmpDir + "/" + chartFilename;//tmpDir是虚拟出的路径
	...
	public void getStatisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(existResultset.equals("1")){			//如果是新查询
            ...
            showDebug(module+"_"+sub+"_statistic_record_result");
