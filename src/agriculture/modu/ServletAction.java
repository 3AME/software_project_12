...
import agriculture.dao.Modu;
import agriculture.dao.ModuDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String moduleName=request.getParameter("MODULE_NAME");//查询功能使用，只有这一个
        String categoryId=request.getParameter("CATEGORY_ID");
        String parentCategoryId=request.getParameter("PARENT_CATEGORY_ID");
        String hreflink=request.getParameter("HREFLINK");
        ...
        if(existResultset.equals("1")){
            ...
            //如果是新查询
            ModuDao moduDao=new ModuDao();
            jsonObj=moduDao.getRecord(query);
            session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
            System.out.println("存放数据集的session名："+module+"_"+sub+"_get_record_result");
        }
    ...
    private JSONObject getResultSetNavigateId(String id,String index,JSONObject json) throws NumberFormatException, JSONException{
        ...
        System.out.println("getResultSetNavigateId"+jsonObj);
        return jsonObj;
    ...
    public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String moduleName=request.getParameter("MODULE_NAME");//查询功能使用
        String categoryId=request.getParameter("CATEGORY_ID");
        String parentCategoryId=request.getParameter("PARENT_CATEGORY_ID");
        String hreflink=request.getParameter("HREFLINK");
        String fileId=request.getParameter("FILE_ID");
        ...
        if(dbName!=null){
            ...
            ModuDao moduDao=new ModuDao();
            Modu modu=new Modu();
            modu.setAction(action);
            modu.setDbName(dbName);

            modu.setModuleName(moduleName);
            modu.setCategoryId(categoryId);
            modu.setParentCategoryId(parentCategoryId);
            modu.setHreflink(hreflink);
            modu.setFileId(fileId);
            modu.setFileName(fileName);
            modu.setFilePath(filePath);



            modu.setCreator(creator);
            modu.setCreateTime(createTime);
            modu.setRoleId(roleId);

            jsonObj=moduDao.addRecord(modu);
            ylxLog.log("用户 "+creator+" 于 "+createTime+" 添加了 ["+module+"]["+sub+"] 记录，权限名为["+fileName+"]","添加记录",module,sub);
        }
        boolean isAjax=true;
        if(request.getHeader("x-requested-with")==null){isAjax=false;}	//判断是异步请求还是同步请求
    ...
    public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id!=null && dbName!=null){
            ...
            modu.setCreator(creator);
            modu.setCreateTime(createTime);
            jsonObj=moduDao.modifyRecord(modu);
...
//            response.sendRedirect(url);
    ...
    public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
            ModuDao moduDao=new ModuDao();
            jsonObj=moduDao.deleteRecord(action,dbName,ids,creator,createTime);
    ...
    public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(dbName!=null){
            ...
            String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
            ModuDao moduDao=new ModuDao();
            jsonObj=moduDao.setRecordTop(action,dbName,type,userId,id);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        boolean isAjax=true;
    ...
    public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String creator=(String)session.getAttribute("user_name");
        String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        //String exportFilePathName="C:\\upload\\agriculture\\export\\modu_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
        String exportFilePathName="C:\\download\\XM18\\";
        String filename="modu_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
    ...
    public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
        if(jsonObj!=null){
            ...
            //需要观察json里的格式
            String[] cols={"role_id⊙text⊙id","time_interval⊙text⊙MODULE_NAME","count⊙text⊙role_id","color⊙text⊙CATEGORY_ID","color_name⊙text⊙PARENT_CATEGORY_ID",
                    "color_name⊙text⊙FILE_PATH","color_name⊙text⊙HREFLINK","color_name⊙text⊙FILE_ID","color_name⊙text⊙FILE_NAME","color_name⊙text⊙create_time"
                    };	//name⊙type⊙nick
