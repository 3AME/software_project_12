...
! function($) {
    ...
    Datetimepicker.prototype = {
        ...
        fill: function() {
            ...
            //添加datetimepicker 下拉框，如果配置要选择秒，否则不添加
            var addSec = /,s{1,2},/.test(',' + this.format.parts.join(',') + ','),
                sSec =
                addSec ? '<select style="width:100%">' : '';
            this.addSec = addSec;
            if (addSec) {
                var orgSec = this.viewDate.getSeconds();
                for (var _i = 0; _i < 60; _i++) sSec += '<option value="' + _i + '"' + (_i ==
                    orgSec ? ' selected' : '') + '>' + (_i < 10 ? '0' : '') + _i + '</option>';
                sSec += '</select>';
            }
